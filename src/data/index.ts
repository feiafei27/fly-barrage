...
export function generateBarrageData(videoId: number, config: generateBarrageConfig) {
  ...
  const textSections = [
    ...
    `${videoId}：不是道人来引笑`,
    `${videoId}：周情孔思正追寻`,
    `${videoId}：浮云一百八盘萦，落日四十八渡明。`,
    `${videoId}：得即高歌失即休，多愁多恨亦悠悠。`,
    `${videoId}：今朝有酒今朝醉，明日愁来明日愁。`,
    `${videoId}：生民何计乐樵苏。`,
    `${videoId}：待到秋来九月八，我花开后百花杀。`,
    `${videoId}：侯门一入深似海，从此萧郎是路人。`,
    `${videoId}：崆峒访道至湘湖，万卷诗书看转愚。`,
    `${videoId}：手帕蘑菇及线香，本资民用反为殃。`,
    `${videoId}：清风两袖朝天去，免得闾阎话短长。`,
    `${videoId}：春风得意马蹄疾，一日看尽长安花。`,
    `${videoId}：残灯无焰影幢幢，此夕闻君谪九江。`,
    `${videoId}：垂死病中惊坐起，暗风吹雨入寒窗。`,
    `${videoId}：新年快乐`,
    `${videoId}：更好的一年`,
    `${videoId}：继续努力`,
    `${videoId}：这泰裤辣`,
